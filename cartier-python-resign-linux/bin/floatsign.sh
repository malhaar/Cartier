...
# Copyright (c) 2017 PYW.CN
# http://www.pyw.cn/
#
# Please let us know about any improvements you make to this script!
# ./floatsign source "iPhone Distribution: Name" uuid -p "path/to/profile" [-d "display name"]  [-e entitlements] [-k keychain] [-P keychain-password] [-keep] [-b "BundleIdentifier"] outputIpa
#
#
...
# set unique temp sign work dir
TEMP_DIR=".${UUID}_floatsignTemp"
...
then
    ...
    #upgrade set command `security unlock "$KEYCHAIN"` -> `security unlock-keychain -p "$KEYCHAIN_PASSWORD" "$KEYCHAIN"`
...
echo "$TEMP_DIR/Payload/$APP_NAME"
arrayA=($(find "$TEMP_DIR/Payload/$APP_NAME" -d -mindepth 1 \( -name "*.app" -or -name "*.appex" \)))

indices=( ${!arrayA[@]} )
for ((i=${#indices[@]} - 1; i >= 0; i--)) ; do
    log "Resigning nested application: '${arrayA[indices[i]]}'"
    resign "${arrayA[indices[i]]}" NESTED
done
